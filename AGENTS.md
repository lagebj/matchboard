...
**Current status**: `src/lib/situational/` implements the resolver, candidate/decision contracts, the situation-policy adapter (with a safe degraded fallback — never suppresses, never throws, never uses `AUTO`), and the projection service. Four candidate providers exist: `assistantWorkItemsToCandidates()` (adapting existing `AssistantWorkItem`s), `createPlanIntegrityCandidateProvider()` (one candidate per `computeRoundPlanIntegrity()` signal, reusing `AssistantCommandCentre.roundPlanIntegrities`), `opportunityGapRowsToCandidates()` (the first real `LONG_TERM` source, adapting `getOpportunityGap()` rows), and `createLiveSessionCandidateProvider()` (detects a stalled live-reporting session — active with no heartbeat in 10+ minutes — from `AssistantCommandCentre.activeLiveSessions`); none recompute domain data the caller hasn't already loaded. Today's hero "Next action" (`/o/{orgSlug}/today`) is selected by the projection, not `CATEGORY_PRIORITY` array order. A first Matchday-mobile slice exists — `MatchdayContextBanner` shows match identity + a live/kickoff status pill + one "Follow live"/"Open match" navigation action above the hero whenever the projection infers `MATCHDAY`. A first Next slice exists too — `NextRoundReadinessSection` lists any round with a blocked/decision-required signal and deep-links to the (unchanged) Round Board. A first Long-term slice exists on a second page beyond Today — the Opportunity Gap insights route resolves a `LONG_TERM` situation and shows a "Situational summary", proving the projection infrastructure is reused across pages, not Today-only. Today's grouped sections now carry situational treatment too — a non-promoted item is annotated "Lower priority right now" and sorted after non-deferred items within its group via a stable sort (never hidden or dropped; Blocked/Decision-required items are always excluded from both the annotation and the reorder) — the metric-tile row above them remains fully unfiltered, and no decision is resolvable inline (navigation only so far). The hero's "no decisions" case is also now correctly distinguished from "no projection at all": a projection that legitimately concludes nothing should be featured (every candidate suppressed/deferred) no longer falls back to raw category order, and its empty state shows different copy for `LIVE` vs. `READY`. See `docs/domain/situational-decision-support.md`'s Status section before assuming any further integration exists.
