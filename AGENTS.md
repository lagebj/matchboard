...
**Never mix scoped `run()`-style tenant calls (`runWithTenantOrganisationId()`, `withTenantContext()`) with a later `setTenantOrganisationId()`/`enterWith()` call in the same request continuation (ARR-0029 "Bug 2b").** This is a real, empirically-reproduced Node.js `AsyncLocalStorage` defect, not a hypothesis: `enterWith()` called after one or more earlier `run()` call exits silently loses the context under concurrent request load (proven with a standalone timer-interleaving stress script — 100% loss with >=2 prior `run()` calls, 0% with either `run()` alone or `enterWith()` alone, under the same concurrency). It broke `requireActorContext()` for nearly every authenticated page under real CI traffic, intermittently — not deterministically, which is what makes it dangerous to miss in a quick manual check. The fix pattern: within a single request's auth-resolution call graph, call `setTenantOrganisationId()` exactly **once**, as early as the organisation identity is known, and have every later query in that call graph (`requireActorContext()`, `resolveOrganisationAccess()`, `getEffectiveGroupAccess()`) rely on that single already-set context — never wrap a later query in its own scoped `run()`. Conversely, a function that does its own `run()`-scoped work and returns without any caller-visible `enterWith()` afterward (e.g. `resolveOrgFilterForMachine()`, `recordEventForActor()`, which wraps its *entire* remaining body in one `run()` call) remains safe.
