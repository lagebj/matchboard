...
- Squads are operational without requiring a mandatory lock step; locking is an optional advisory action
- When satisfied, the coach may lock squads via `lockEventSquadsAction`, which runs validation first
...
- Locked squads can be unlocked to DRAFT via `unlockEventSquadsAction`
- The Assistant surfaces `event_squads_ready` work items when squads exist for an event
