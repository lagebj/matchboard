...
- Product boundary: private coach-facing football planning app on Vercel with Neon PostgreSQL persistence; not a generic club platform, not parent comms, not a public player ranking system.
- Behavioral source of truth: `features/matchboard.feature`.
- If code, UI, database, feature files, README, or agent guidance disagree, resolve the mismatch before completion.
...
- the relevant module(s) in `docs/agents/README.md`
- any active ADR/ARR documents governing the affected domain

The root `AGENTS.md` is intentionally small. Detailed product and security guidance lives under `docs/agents/` so the repository does not trigger the static-context regression caused by huge auto-loaded instruction files.

## Required skill names

Use the relevant skill when the task matches it. Do not eagerly inline full skill bodies into every session.

- `git-branch-commit-pr` — all coding work, branches, commits, PRs
- `adr-governance` — architecture-affecting or durable design changes
- `architectural-residue-records` — inherited mismatch or legacy residue work
- `frontend-ui-engineering` — browser/UI, navigation, presentation, accessibility
- `application-security-engineering` — auth, authz, tenant isolation, secrets, provider config
...
- Treat `features/matchboard.feature` as the authoritative behavior contract when code and docs conflict.
- Keep docs, tests, feature files, and agent instructions aligned for any product or workflow change.
- Authentication is not authorization; authorization remains server-side and deny-by-default.
- Raw SQL concatenation or `*_Unsafe` operations are forbidden in app code.
...
Detailed Matchboard product, domain, security, UX, and operations guidance is intentionally organized under `docs/agents/` and loaded by relevance. Do not force the entire historical rule dump into every session.

The module index is at `docs/agents/README.md`.
