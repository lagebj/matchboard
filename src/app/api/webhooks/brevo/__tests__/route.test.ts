...
describe("Brevo webhook route bearer auth", () => {
  ...
  async function getHandler(tokenEnv?: string) {
  ...
  it("rejects request without authorization header when token is configured", async () => {
    const handler = await getHandler(VALID_TOKEN);
  ...
  it("allows unauthenticated request in non-production when no token is configured", async () => {
    vi.doMock("@/lib/email/webhook-handler", () => ({
      processBrevoWebhookEvents: vi.fn().mockResolvedValue({ processed: 1, errors: [] }),
    }));
...
describe("timingSafeEqual for bearer comparison", () => {
  it("prevents timing attacks using constant-time comparison", () => {
  ...
  it("rejects tokens with different lengths without comparison", () => {
    const short = "Bearer short";
    const long = "Bearer test-bearer-token-12345678";
