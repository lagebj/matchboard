...
describe("/api/test-agent/seed-draft-match", () => {
  ...
  function makeRequest(body: unknown) {
    return new Request("http://localhost:3333/api/test-agent/seed-draft-match", {
      method: "POST",
  ...
  it("rejects invalid JSON body", async () => {
    const { POST } = await import("../route");
    const request = new Request("http://localhost:3333/api/test-agent/seed-draft-match", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: "not json",
    });
