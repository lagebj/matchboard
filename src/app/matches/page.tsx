...
function buildAssistantManagerNote(input: {
  ...
  highlightWeek:
    | {
        ...
        label: string;
        matches: Array<{ latestSelectionStatus: SelectionStatus | null }>;
      }
    | null;
  nextActionMatch:
    | {
  ...
  if (!input.highlightWeek) {
    return { detail: "Create the first match and the week flow will appear here.", title: "No week is active yet." };
  }

  if (input.nextActionMatch) {
    return { detail: `${input.highlightWeek.label} is the live lane. ${input.nextActionMatch.team.name} vs. ${input.nextActionMatch.opponent} is the next unresolved call.`, title: "Assistant manager says: stay with the active week." };
  }

  if (input.openWeekCount > 0) {
    return { detail: `${input.draftCount} draft match${input.draftCount === 1 ? "" : "es"} still sit in the board. Clean those up before adding new work.`, title: "Assistant manager says: finish the saved drafts." };
  }

  return { detail: `${input.highlightWeek.label} is locked. Review history or reset a week when you want a fresh restart.`, title: "Assistant manager says: the queue is fully locked." };
}
...
function getMovementOwnTeamStatus(input: {
  ...
  playerById: Map<
    string,
    { coreTeamId: string; coreTeam: { name: string } }
  >;
  ...
  if (!player) {
    return { tone: "neutral", text: "Own-team context unavailable" } as const;
  }
  const coreMatch = input.weekMatches.find((m) => m.team.id === player.coreTeamId);
  if (!coreMatch) {
    return { tone: "neutral", text: `No ${player.coreTeam.name} match this week` } as const;
  }
  if (coreMatch.id === input.matchId) {
    return { tone: "neutral", text: "Own-team match" } as const;
  }
  if (coreMatch.latestSelectionStatus === null) {
    return { tone: "neutral", text: `${player.coreTeam.name} not saved yet` } as const;
  }
  const selectedInCoreMatch = input.selectedPlayerIdsByMatchId.get(coreMatch.id)?.includes(input.playerId) ?? false;
  return selectedInCoreMatch
    ? { tone: "settled", text: `Still selected for ${player.coreTeam.name}` }
    : { tone: "warning", text: `Left out of ${player.coreTeam.name}` };
}
...
export default async function MatchesPage({ searchParams }: MatchesPageProps) {
  ...
  const [matches, teams, players, selections, roleBucketSelections] = await Promise.all([
    db.match.findMany({
      include: { team: { select: { id: true, name: true, developmentSlots: true, targetSquadSize: true } } },
...
include: { player: { select: { firstName: true, id: true, lastName: true } } },
    ...
    db.selection.findMany({
      ...
      include: {
        ...
        match: { select: { id: true, team: { select: { id: true, name: true } } } },
      },
      orderBy: [{ role: "asc" }],
    }),
  ]);
...
const movementPlayersByMatchId = new Map<string, Array<{ playerId: string; playerName: string; sourceTeamName: string; targetTeamName: string }>>();
  ...
  const weekGroups = getMatchWeekGroups(enrichedMatches, latestSelectionStatusByMatchId).map((w) => ({
    ...w,
    coverage: getWeeklyPlayerCoverage(players,
      w.matches.map((mt) => ({ id: mt.id, opponent: mt.opponent, team: mt.team })),
      selectedPlayerIdsByMatchId,
    ),
  }));

  const nextActionMatch = enrichedMatches.find((m) => m.latestSelectionStatus !== SelectionStatus.FINALIZED) ?? null;
  const openWeekCount = weekGroups.filter((w) => !w.isFullyFinalized).length;
  const finalizedWeekCount = weekGroups.filter((w) => w.isFullyFinalized).length;
  const draftCount = enrichedMatches.filter((m) => m.latestSelectionStatus === SelectionStatus.DRAFT).length;
  const highlightWeek = weekGroups.find((w) => !w.isFullyFinalized) ?? weekGroups[0] ?? null;
    ...
    <main className="flex min-h-full flex-col gap-6 text-foreground">
          ...
          <div className="grid gap-3">
            ...
            <div className="grid gap-3 sm:grid-cols-3 xl:grid-cols-1">
              <div className="rounded-[1.5rem] border app-hairline bg-[rgba(255,255,255,0.025)] p-4">
                <p className="text-[11px] font-semibold uppercase tracking-[0.18em] app-copy-muted">Open weeks</p>
                <p className="mt-2 text-2xl font-semibold text-zinc-50">{openWeekCount}</p>
              </div>
              <div className="rounded-[1.5rem] border app-hairline bg-[rgba(255,255,255,0.025)] p-4">
                <p className="text-[11px] font-semibold uppercase tracking-[0.18em] app-copy-muted">Finalized weeks</p>
      ...
      <div className="flex flex-col gap-3">
        ...
        {humanReview === "recommended" && (
          <div className="rounded-2xl border border-[rgba(208,176,127,0.34)] bg-[rgba(208,176,127,0.14)] px-4 py-4 text-sm text-[var(--foreground)]">
            <p className="font-medium text-[var(--warning)]">Human review recommended</p>
            <p className="mt-1 app-copy-soft">This round has more warnings than the configured threshold. Review the warnings before proceeding.</p>
          </div>
        )}
      ...
      {teams.length === 0 ? (
        <section className="app-panel rounded-[1.75rem] p-6">
          ...
          <p className="mt-1 text-sm leading-6 app-copy-soft">Create a team before adding new matches.</p>
          <div className="mt-4">
            <Link
        ...
        <>
          <section className="app-panel rounded-[1.75rem] p-6">
            <div className="flex items-start justify-between gap-4">
              <div>
                <p className="text-[11px] font-semibold uppercase tracking-[0.22em] text-[var(--accent-strong)]">Weekly Workflow</p>
                <h2 className="mt-2 text-xl font-semibold text-zinc-50">One card per week</h2>
                <p className="mt-2 text-sm app-copy-soft">Keep the live week brightest. Coverage, movement, and next actions stay on the same card.</p>
              </div>
              <Link
            ...
            <div className="mt-6 grid gap-4 xl:grid-cols-2">
              {weekGroups.length > 0 ? (
                weekGroups.map((week, index) => {
                  const warningCount = week.coverage.filter((r) => r.severity === "warning").length;
                  const infoCount = week.coverage.length - warningCount;
                  const weekMatchIds = week.matches.map((m) => m.id);
                  const weekMovementCount = week.matches.reduce(
                    (sum, m) => sum + (movementPlayersByMatchId.get(m.id)?.length ?? 0),
                    0,
                  );
                  const unresolvedMatch = week.matches.find((m) => m.latestSelectionStatus !== SelectionStatus.FINALIZED);
                  ...
                  return (
                    <div key={week.label} className={`rounded-[1.5rem] border p-4 ${cardTone}`}>
                      <div className="flex items-start justify-between gap-3">
                        ...
                        <div className="flex flex-wrap items-center justify-end gap-2">
                          ...
                          {week.matches[0]?.matchRoundId && (
                            <Link
                              className="inline-flex h-9 items-center rounded-full border app-hairline px-3 text-xs font-medium app-copy-soft hover:bg-[rgba(255,255,255,0.05)] hover:text-zinc-50"
                              href={`/rounds/${week.matches[0].matchRoundId}`}
                            >
                              Round board
                            </Link>
                          )}
                      ...
                      <div className="mt-4 grid gap-3 sm:grid-cols-4">
                        ...
                        <div className="rounded-2xl border app-hairline bg-[rgba(0,0,0,0.14)] px-3 py-3">
                          <p className="text-[11px] font-semibold uppercase tracking-[0.18em] app-copy-muted">Info</p>
                          <p className="mt-2 text-lg font-semibold text-zinc-50">{infoCount}</p>
                        </div>
                        <div className="rounded-2xl border app-hairline bg-[rgba(0,0,0,0.14)] px-3 py-3">
                          <p className="text-[11px] font-semibold uppercase tracking-[0.18em] app-copy-muted">Movers</p>
                          ...
                          >
                            <div className="flex flex-wrap items-start justify-between gap-3">
                              ...
                              <span className="rounded-full border app-hairline px-3 py-1 text-[11px] uppercase tracking-[0.18em] app-copy-soft">
                                {match.team.developmentSlots > 0 ? "Development open" : "Development closed"}
                              </span>
                            </div>
                      ...
                      <div className="mt-4 flex flex-wrap gap-2">
                        <form action={recalculateMatchesAction}>
                          ...
                          <button className="h-10 rounded-full border app-hairline bg-[rgba(255,255,255,0.04)] px-4 text-sm font-medium app-copy-soft hover:bg-[rgba(255,255,255,0.08)] hover:text-zinc-50" type="submit">
                        ...
                        <form action={finalizeMatchesAction}>
                          ...
                          <div className="flex flex-wrap items-center gap-2">
                            <input
                              className="h-10 rounded-full border app-hairline bg-[rgba(255,255,255,0.04)] px-4 text-sm app-copy-soft focus:outline-none focus:ring-1 focus:ring-[var(--accent-strong)]"
                              name="overrideReason"
                              placeholder="Override reason (if required)"
                              type="text"
                            />
                            <button className="h-10 rounded-full border app-hairline bg-[rgba(255,255,255,0.04)] px-4 text-sm font-medium app-copy-soft hover:bg-[rgba(255,255,255,0.08)] hover:text-zinc-50" type="submit">
                              Finalize week
                      ...
                      <div className="mt-4 grid gap-3 xl:grid-cols-2">
                        <div className="rounded-2xl border app-hairline bg-[rgba(0,0,0,0.12)] px-4 py-4">
                          <p className="text-[11px] font-semibold uppercase tracking-[0.18em] app-copy-muted">Week coverage</p>
                          {week.coverage.length > 0 ? (
                              ...
                              {week.coverage.slice(0, 3).map((row) => (
                                <div key={row.playerId} className="text-sm">
                                  <p className="font-medium text-zinc-100">
                                    {row.playerName}
                                    <span className="ml-2 text-xs uppercase tracking-[0.18em] app-copy-muted">{row.severity}</span>
                                  </p>
                            ...
                            <p className="mt-3 text-sm app-copy-soft">No uncovered active available players for this week.</p>
                          )}
                        </div>
                        <div className="rounded-2xl border app-hairline bg-[rgba(0,0,0,0.12)] px-4 py-4">
                          <p className="text-[11px] font-semibold uppercase tracking-[0.18em] app-copy-muted">Selection movement</p>
...
{match.team.name} vs. {match.opponent}
                                  ...
                                  {movementPlayers.length > 0 ? (
                                    <div className="mt-3 flex flex-col gap-2">
                                      {movementPlayers.map((player) => {
                                        ...
                                        return (
                                          <div key={player.playerId} className="rounded-xl border app-hairline bg-[rgba(255,255,255,0.03)] px-3 py-3">
                                            <div className="flex flex-wrap items-center gap-2">
                                              <span className="font-medium text-zinc-100">{player.playerName}</span>
                                              <span className="text-xs app-copy-soft">{player.sourceTeamName} to {player.targetTeamName}</span>
                                            </div>
                                            <div className="mt-2 flex flex-wrap gap-2">
                                              <span
                                                className={`rounded-full border px-3 py-1 text-[11px] uppercase tracking-[0.18em] ${ownTeamStatus.tone === "warning"
                                    ...
                                    <p className="mt-2 app-copy-soft">
                                      {match.latestSelectionStatus ? "No floating players saved in this match." : "No saved draft yet."}
                                    </p>
                                  )}
                                </div>
                              );
          ...
          <section className="app-panel rounded-[1.75rem] p-6">
            <div className="flex items-start justify-between gap-4">
              <div>
                <p className="text-[11px] font-semibold uppercase tracking-[0.22em] text-[var(--accent-strong)]">Team Columns</p>
                <h2 className="mt-2 text-xl font-semibold text-zinc-50">One column per team match, grouped by role bucket</h2>
                <p className="mt-2 text-sm app-copy-soft">See cross-team consequences at a glance. Each column shows core, support, backfill, development, drops, and unavailable players.</p>
              </div>
            </div>

            {(() => {
              type RoleBucketKey = "CORE" | "SUPPORT" | "BACKFILL" | "DEVELOPMENT" | "CONFIDENCE_REBUILD" | "CORE_MATCH_DROP" | "REDUCED_MATCH_LOAD_DROP" | "MANUAL_OVERRIDE";

              const BUCKET_ORDER: RoleBucketKey[] = [
                "CORE",
                "SUPPORT",
                "BACKFILL",
                "DEVELOPMENT",
              ...
              const BUCKET_LABELS: Record<RoleBucketKey, string> = {
                CORE: "Core",
              ...
              const selectionsByMatchId = new Map<string, typeof roleBucketSelections>();
              for (const sel of roleBucketSelections) {
                const existing = selectionsByMatchId.get(sel.matchId) ?? [];
                existing.push(sel);
                selectionsByMatchId.set(sel.matchId, existing);
              }

              const matchesWithSelections = enrichedMatches.filter(
                (m) => m.latestSelectionStatus === SelectionStatus.DRAFT || m.latestSelectionStatus === SelectionStatus.FINALIZED,
              );

              if (matchesWithSelections.length === 0) {
                return (
                  <div className="mt-6 rounded-2xl border app-hairline bg-[rgba(255,255,255,0.025)] px-4 py-5 text-sm app-copy-soft">
                    No matches have draft or finalized selections yet. Generate a selection to see team columns with role buckets.
                  </div>
                );
              }
              ...
              return (
                <div className="mt-6 -mx-2 overflow-x-auto px-2 pb-2">
                  <div className="flex gap-4" style={{ minWidth: `${Math.max(matchesWithSelections.length * 22, 44)}rem` }}>
                    {matchesWithSelections.map((match) => {
                      const matchSels = selectionsByMatchId.get(match.id) ?? [];
                      const buckets = new Map<RoleBucketKey, typeof matchSels>();
                      for (const sel of matchSels) {
                        const key = sel.role as RoleBucketKey;
                      ...
                      const targetSquad = match.team?.targetSquadSize ?? 7;

                      return (
                        <div key={match.id} className="flex-1 min-w-[20rem] rounded-[1.5rem] border app-hairline bg-[rgba(255,255,255,0.025)] p-4">
                          ...
                          <div className="mt-3 flex items-center gap-2">
                            ...
                            <span className={`rounded-full border px-2 py-0.5 text-[10px] uppercase tracking-[0.14em] ${match.latestSelectionStatus === SelectionStatus.FINALIZED ? "border-[rgba(140,167,146,0.28)] bg-[rgba(140,167,146,0.12)] text-[var(--accent-strong)]" : "border-[rgba(208,176,127,0.24)] bg-[rgba(208,176,127,0.12)] text-[var(--warning)]"}`}>
                              {match.latestSelectionStatus === SelectionStatus.FINALIZED ? "Finalized" : "Draft"}
                            </span>
                          </div>

                          <div className="mt-4 flex flex-col gap-3">
                            {BUCKET_ORDER.map((bucketKey) => {
                              const bucketSels = buckets.get(bucketKey);
                              if (!bucketSels || bucketSels.length === 0) return null;

                              const isDrop = bucketKey === "CORE_MATCH_DROP" || bucketKey === "REDUCED_MATCH_LOAD_DROP";
                              const isSupport = bucketKey === "SUPPORT" || bucketKey === "BACKFILL";

                              return (
                                <div key={bucketKey} className={`rounded-xl border p-3 ${isDrop ? "border-[rgba(185,128,119,0.2)] bg-[rgba(185,128,119,0.04)]" : isSupport ? "border-[rgba(208,176,127,0.2)] bg-[rgba(208,176,127,0.04)]" : "border app-hairline bg-[rgba(0,0,0,0.08)]"}`}>
                                  <p className="text-[10px] font-semibold uppercase tracking-[0.16em] app-copy-muted">
                                    {BUCKET_LABELS[bucketKey]} ({bucketSels.length})
                                  </p>
                                  <div className="mt-2 flex flex-col gap-1.5">
                                    {bucketSels.map((sel) => {
                                      const player = sel.player;
                                      const isDifferentCoreTeam = player.coreTeam.id !== match.team.id;
                                      return (
                                        <Link
                                          key={sel.id}
                                          className="flex items-center justify-between gap-2 rounded-lg border app-hairline bg-[rgba(0,0,0,0.12)] px-2.5 py-1.5 text-sm hover:bg-[rgba(255,255,255,0.04)]"
                                        ...
                                        >
                                          ...
                                          <span className="shrink-0 text-[10px] uppercase tracking-[0.12em] app-copy-muted">
                                            {isDifferentCoreTeam ? player.coreTeam.name : player.primaryPosition}
                                          </span>
                                        </Link>
                                      );
                                    })}
                            ...
                            {matchSels.length === 0 && (
                              <p className="text-sm app-copy-soft">No selections yet for this match.</p>
                            )}
                          </div>
                        </div>
                      );
          ...
          <section className="app-panel rounded-[1.75rem] p-6">
            <MatchTable
              matches={enrichedMatches.map((match) => ({
                ...match,
                availableForDevelopmentSlot: match.team?.developmentSlots > 0,
                homeAway: match.homeAway ?? "HOME",
                squadSize: 0,
                team: match.team,
                deleteAction: deleteMatchAction.bind(null, match.id),
                updateDevelopmentAvailabilityAction: updateMatchDevelopmentAvailabilityAction.bind(null, match.id),
              }))}
