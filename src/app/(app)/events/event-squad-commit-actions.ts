...
export async function validateEventSquadsBeforeLock(
  eventId: string,
  ...
  if (squads.length === 0) {
    issues.push({
      code: "no_squads",
      severity: "blocking",
      message: "Event has no squads to lock.",
    });
    return { valid: false, issues };
  }
...
export async function lockEventSquadsAction(eventId: string) {
  const coach = await requireCoachAccess();
  const orgFilter = await resolveOrgFilterForUser(coach.id ?? "");

  await requireEventOrgAccess(eventId, orgFilter);

  const validation = await validateEventSquadsBeforeLock(eventId);
  ...
  if (!validation.valid) {
    ...
    return {
      ...
      error: "Cannot lock squads: blocking issues found.",
...
export async function unlockEventSquadsAction(eventId: string) {
  const coach = await requireCoachAccess();
  const orgFilter = await resolveOrgFilterForUser(coach.id ?? "");

  await requireEventOrgAccess(eventId, orgFilter);

  const lockedCount = await db.eventSquad.count({
  ...
  if (lockedCount === 0) {
    ...
    return {
      success: false as const,
      error: "No locked squads to unlock.",
    };
  }
...
export async function confirmEventSquadsAction(eventId: string) {
  return lockEventSquadsAction(eventId);
}

export async function unconfirmEventSquadsAction(eventId: string) {
  return unlockEventSquadsAction(eventId);
}

export const validateEventSquadsBeforeCommit = validateEventSquadsBeforeLock;