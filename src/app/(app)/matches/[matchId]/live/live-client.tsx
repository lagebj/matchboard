...
export function LiveMatchClient({ matchId, matchInfo }: LiveMatchClientProps) {
  ...
  useEffect(() => {
    async function loadPreMatch() {
      ...
      if (result.success && result.data) {
        ...
        setError(result.success === false ? result.error : "Failed to load match data");
      }
      setLoading(false);
    }
    loadPreMatch();
  ...
  useEffect(() => {
    ...
    async function fetchEvents() {
      const result = await getRecentEventsAction(matchId, 10);
      if (result.success && result.data) {
  ...
  const handleStartSession = useCallback(async () => {
    ...
    if (result.success && result.data) {
      ...
      setError(result.success === false ? result.error : "Failed to start session");
    }
    setIsPending(false);
  }, [matchId]);
  ...
  const handleEndSession = useCallback(async () => {
    ...
    if (result.success) {
      ...
      setError(result.success === false ? result.error : "Failed to end session");
    }
    setIsPending(false);
  }, [sessionId]);
  ...
  const recordEvent = useCallback(
    async (eventType: string, extra?: { playerId?: string; secondaryPlayerId?: string; period?: string; matchSeconds?: number; correctionType?: string; correctsEventId?: string; payload?: Record<string, unknown> }) => {
      ...
      if (!result.success) {
        setError(result.success === false ? result.error : "Failed to record event");
      }
      setIsPending(false);
    },
    [matchId, sessionId],
  ...
  const handlePeriodAdvance = useCallback(() => {
    ...
    const periodEvents: Record<string, string> = {
      BEFORE: "MATCH_START",
      FIRST_HALF: "PERIOD_START",
      HALF_TIME: "PERIOD_END",
      SECOND_HALF: "PERIOD_START",
      EXTRA_FIRST_HALF: "PERIOD_START",
      EXTRA_HALF_TIME: "PERIOD_END",
      EXTRA_SECOND_HALF: "PERIOD_START",
      FULL_TIME: "MATCH_END",
    };
  ...
  }, [clock, sessionId, recordEvent, goalFlowTimer]);
  ...
  const homeAwayLabel = matchInfo.homeAway === "HOME" ? "vs" : "@";
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-zinc-950 p-6 space-y-6">
        <div className="text-center">
          <h1 className="text-2xl font-bold text-zinc-100">{matchInfo.teamName}</h1>
          <p className="text-zinc-400 mt-1">
            {homeAwayLabel} {matchInfo.opponent}
          </p>
          <p className="text-sm text-zinc-500 mt-1">{matchInfo.roundName ?? ""}</p>
        </div>
        <button
    ...
    <div className="min-h-screen bg-zinc-950 text-zinc-100 flex flex-col">
      ...
      <div className="px-4 pt-3 pb-2 border-b border-zinc-800">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-lg font-bold">{matchInfo.teamName}</h1>
            <p className="text-sm text-zinc-400">{homeAwayLabel} {matchInfo.opponent}</p>
          </div>
          <button
      ...
      <div className="px-4 py-3 bg-zinc-900 border-b border-zinc-800">
        <div className="flex items-center justify-between">
          <div className="text-center flex-1">
            <div className="text-3xl font-bold">{matchInfo.teamName}</div>
          ...
          <div className="text-center px-4">
            ...
            >
              {isPlayingPeriod(clock.period) ? "End period" : isOver ? "Match ended" : "Next period"}
            </button>
          </div>
          <div className="text-center flex-1">
            <div className="text-3xl font-bold text-zinc-400">{matchInfo.opponent}</div>
