...
function PlayerCard({ player }: { player: PlayerAssignmentBoardPlayer }) {
  return (
    <div className="group rounded border border-zinc-700/50 bg-zinc-800/60 px-2.5 py-2 text-sm transition-colors hover:border-zinc-600 hover:bg-zinc-800">
      ...
      <div className="mt-1 opacity-0 group-hover:opacity-100 transition-opacity">
        <MoveToSelect player={player} />
      </div>
    </div>
  );
}

function MoveToSelect({ player }: { player: PlayerAssignmentBoardPlayer }) {
    ...
    <BoardContextProvider board={board} onMove={loadBoard}>
      <div className="flex flex-col gap-4">
        <div>
          ...
          <p className="text-xs text-zinc-500 mt-0.5">Assign players to teams by dragging them between columns.</p>
        </div>
            ...
            <div className="grid grid-cols-1 gap-4 lg:grid-cols-4">
              {board.teams.map((team) => (
                <div key={team.teamId} className="flex flex-col gap-2">
                  <div className="flex items-center justify-between">
                    <h2 className="text-sm font-semibold text-zinc-200">{team.name}</h2>
                    <span className="rounded bg-zinc-800 px-1.5 py-0.5 text-[10px] text-zinc-400">{team.players.length} players</span>
                  </div>
                  <div
                    id={team.teamId}
                    className="flex min-h-[200px] flex-col gap-1.5 rounded-lg border border-zinc-800 bg-zinc-900/30 p-2"
                  >
                    {team.players.length === 0 ? (
                      <p className="py-4 text-center text-xs text-zinc-600">Drop players here</p>
                    ) : (
                      team.players.map((player) => (
                        <div key={player.playerId} id={player.playerId} className="cursor-grab active:cursor-grabbing">
              ...
              <div className="flex flex-col gap-2">
                <div className="flex items-center justify-between">
                  ...
                  <span className="rounded bg-zinc-800 px-1.5 py-0.5 text-[10px] text-zinc-500">{board.unassigned.length} players</span>
                </div>
                <div
                  id="__unassigned__"
                  className="flex min-h-[200px] flex-col gap-1.5 rounded-lg border border-dashed border-zinc-700 bg-zinc-900/20 p-2"
                >
