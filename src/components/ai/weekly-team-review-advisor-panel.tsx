...
import { confirmAiDevelopmentSuggestionAction, dismissAiInsightAction } from "@/app/(app)/o/[orgSlug]/teams/[teamId]/review/ai-insight-actions";
import type { WeeklyTeamReviewAdvisorViewModel, WeeklyTeamReviewDevelopmentSuggestionViewModel } from "@/lib/ai/presentation/weekly-team-review-advisor";

/**
 * "Weekly team review" Advisor block (08_UI_UX_SPEC.md "Weekly review"): folded into the
 * existing Team Review surface, after its deterministic readiness/rule-impact panels — no new
 * top-level navigation item. Header style matches the completed-match/round-board panels
 * (badge + label, no subtitle sentence). A development-suggestion insight gets the same
 * Confirm/Dismiss affordance as the completed-match panel, since `weekly_team_review` may also
 * propose at most one development observation per player (06_AI_CAPABILITY_CONTRACTS.md "5.
 * weekly_team_review").
 */
export function WeeklyTeamReviewAdvisorPanel({
  viewModel,
}: {
  viewModel: Extract<WeeklyTeamReviewAdvisorViewModel, { status: "fresh" }>;
}) {
  ...
  return (
    <Surface padding="md">
      <div className="flex items-center gap-3">
        ...
        <span className="text-sm font-semibold">Weekly team review</span>
      </div>
      ...
      <p className="mt-3 text-xs text-[var(--text-muted)]">Based on last week&apos;s recorded activity</p>
    </Surface>
  );
}

function DevelopmentSuggestionCard({ orgSlug, suggestion }: { orgSlug: string; suggestion: WeeklyTeamReviewDevelopmentSuggestionViewModel }) {
