...
export function CoachHandoverView({ match }: CoachHandoverViewProps) {
  ...
  return (
    ...
    <div className="touchline mx-auto max-w-lg space-y-3 px-3 pb-6 pt-2">
      ...
      {blockedWarnings.length > 0 && (
        <div className="rounded-md border border-[var(--danger)]/40 bg-[var(--danger-subtle)] px-3 py-2">
          <p className="text-sm font-medium text-[var(--danger)]">Blocked</p>
          {blockedWarnings.map((w) => (
            <p key={w.id} className="text-xs text-[var(--danger)]">{w.message}</p>
          ))}
        </div>
      )}

      {decisionWarnings.length > 0 && (
        <div className="rounded-md border border-[var(--warning)]/40 bg-[var(--warning-subtle)] px-3 py-2">
          <p className="text-sm font-medium text-[var(--warning)]">Decision required</p>
          {decisionWarnings.map((w) => (
            <p key={w.id} className="text-xs text-[var(--warning)]">{w.message}</p>
          ))}
        </div>
      )}
        ...
        <TacticalSurface variant="default" padding="sm">
          ...
          <div className="mt-1.5 space-y-1.5">
            {match.plannedRotation.changes.map((c) => (
              <div key={c.id} className="rounded border border-[var(--border-soft)] bg-[var(--surface-raised)] px-2 py-1.5 text-sm">
