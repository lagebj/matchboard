      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-sm">
        ...
        {concernCount > 0 && (
          <div>
            ...
            <p className="text-[var(--warning)] font-medium">{concernCount}{latestConcernDate ? ` · ${latestConcernDate}` : ""}</p>
          </div>
        )}
      </div>
          ...
          {recentMatches.map((m) => (
            ...
            >
              <div className="flex items-center gap-3 min-w-0">
                ...
                <span className="text-[var(--text-muted)] shrink-0">{m.matchDate ? formatKickoffDate(new Date(m.matchDate)) : "—"}</span>
              ...
              <div className="flex items-center gap-3 shrink-0">
                <span className="text-[var(--text-soft)] tabular-nums">{formatScore(m.homeGoals, m.awayGoals)}</span>
