...
export async function computeRoundPlanIntegrity(
      ...
      if (!AVAILABILITY_VALUES_AVAILABLE.has(avail)) {
        ...
        if (avail === "INJURED" || avail === "SICK" || avail === "AWAY" || avail === "UNAVAILABLE") {
          signals.push({
            ...
            title: `Blocked: a selected player is unavailable`,
            currentState: `A player marked ${avail.toLowerCase()} for this round is included in the planned squad.`,
  ...
  for (const [playerId, count] of playerSelectionCountInRound) {
    if (count > 1) {
      ...
      signals.push({
        ...
        matchRoundId,
        playerId,
        title: `Blocked: invalid duplicate planned assignment found for a player`,
        currentState: `A player appears in more than one planned match in this round.`,
  ...
  for (const player of eligibleActivePlayers) {
    ...
    signals.push({
      ...
      matchRoundId,
      playerId: player.id,
      title: `Decision required: an available player has no planned match opportunity this round`,
      currentState: `An available player is not assigned to a match this round.`,
