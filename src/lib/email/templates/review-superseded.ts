...
interface ReviewSupersededData {
...
function renderReviewSuperseded(data: Record<string, unknown>) {
  const d: ReviewSupersededData = {
  ...
  const subject = `Review superseded — ${d.organisationName}`;
...
<body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; line-height: 1.6; color: #1a1a1a; max-width: 600px; margin: 0 auto; padding: 20px;">
  <div style="background-color: #f8f9fa; border-radius: 8px; padding: 32px; margin-bottom: 24px;">
    <h1 style="font-size: 20px; font-weight: 600; margin: 0 0 16px;">Review superseded</h1>
    <p style="margin: 0 0 16px; color: #4a4a4a;">
      Your review request on <strong>${escapeHtml(d.organisationName)}</strong> has been superseded.
    </p>
    <p style="margin: 0 0 16px; color: #4a4a4a;">
  ...
  const textBody = [
    `Review superseded — ${d.organisationName}`,
    "",
    "Your review request has been superseded.",
    "",
...
registerTemplate("REVIEW_SUPERSEDED", renderReviewSuperseded);