...
interface OwnershipAssignedData {
...
function renderOwnershipAssigned(data: Record<string, unknown>) {
  const d: OwnershipAssignedData = {
  ...
  const subject = `You have been assigned ownership on ${d.organisationName}`;
...
<body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; line-height: 1.6; color: #1a1a1a; max-width: 600px; margin: 0 auto; padding: 20px;">
  <div style="background-color: #f8f9fa; border-radius: 8px; padding: 32px; margin-bottom: 24px;">
    <h1 style="font-size: 20px; font-weight: 600; margin: 0 0 16px;">Ownership assigned</h1>
    <p style="margin: 0 0 16px; color: #4a4a4a;">
      <strong>${escapeHtml(d.assignerName)}</strong> (${escapeHtml(d.assignerEmail)}) has assigned you ownership of a work item on <strong>${escapeHtml(d.organisationName)}</strong>.
    </p>
    <p style="margin: 0 0 16px; color: #4a4a4a;">
  ...
  const textBody = [
    `Ownership assigned on ${d.organisationName}`,
    "",
    `${d.assignerName} (${d.assignerEmail}) has assigned you ownership.`,
    "",
...
registerTemplate("OWNERSHIP_ASSIGNED", renderOwnershipAssigned);