...
- Deferred, not silently dropped: "planned continuity/combination exposure" (deriving which partnerships a *plan* would produce, before kickoff) is not yet implemented — `projectPlannedLineup`/`projectPlannedMinutes`/`checkPlannedRotationCoverage` are pure, tested, but still unused by any call site. Wiring them (plus resolving `line`/`lane` for planned starters the same way `actual-timeline.ts` does) is left for a follow-up rather than duplicating the actual-topology engine under time pressure.
