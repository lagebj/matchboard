...
- **UI surfacing (completed in a follow-up change):** a "Repair options" action (Wrench icon, `PlayerChip`'s new `onRepair` prop) on any player chip in a Round Board match column (not the Available column, never on a finalized match) opens a dialog listing the ranked options from `generateEmergencyRepairOptionsAction()`. Applying a chosen option reuses the board's existing remove/add manual-edit actions (the same ones drag/drop and the non-drag Move picker already use) with `overrideReasonCategory: "availability_changed"` — no new mutation path was introduced for this trigger.
