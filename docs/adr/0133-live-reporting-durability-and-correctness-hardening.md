...
|---|--------|------|-----------|
| **H1** ✅ | `seedReportFromLiveSession()` **merges** into an existing DRAFT report instead of bailing: score set only when currently unset, `UNKNOWN` attendance → `PRESENT` for players who appeared, and goals/assists/fair-play/rotations seeded only for categories the report has none of yet; a `REPORTED`/`LOCKED` report is left untouched (`merged: false`). Delivered in PR following this ADR. | med (core report path) | yes — this ADR |
| **H1a** ✅ | H1 follow-up: `seedReportFromLiveSession()` now excludes any `LiveMatchEvent` targeted by an `EVENT_REVERSED` row's `correctsEventId` before deriving the score, scorers, assists and rotation pairs — a reversed `GOAL_FOR` no longer survives into the seeded report (same class of bug as H6a, different code path). The reversal-marker row itself was already excluded (`correctionType: "REVERSAL"`); the fix adds a second query for `correctsEventId`s and filters the working set. Delivered in PR following this ADR. **Not in scope (noted):** an `EVENT_CORRECTED` (`correctionType: "CORRECTION"`) row and the row it supersedes are still both counted — a corrected scorer double-counts; no evidence this fired in the incident, tracked here for a future pass. | low | yes — this ADR |
...
| **H5** ✅ | `canStartLiveReporting()` (`src/lib/matches/can-live-report.ts`, pure + unit-tested) drives the **match detail** "Start live reporting" button: offered while a session is live, or for any not-yet-played, not-yet-reported, non-cancelled match — no longer gated only on `isMatchFinalized(selections)`, and (2026-09-12 refinement) no longer gated on kickoff proximity either. Hidden for cancelled / `done` / `report_incomplete` / `played` (those route through "After match"). Label switches "Start live reporting" ↔ "Live reporting". Round Board has no live entry point today — a per-match column action there is a deliberate deferral (out of scope for the incident fix; the coach reaches live via match detail). | low | no |
...
- **H1a** shipped in the following PR (`seedReportFromLiveSession()` excludes `EVENT_REVERSED`
...
**H5 addendum (2026-09-12):** coach feedback after H5 shipped was that "Start live reporting" was
*still* too easy to lose track of — `LIVE_REPORTING_LEAD_MS` (the one-hour-before-kickoff gate)
meant the button simply wasn't there yet whenever the coach checked the match page earlier than
that, which is a normal and common thing to do (confirming the squad the night before, checking
the page during the day, etc.). `LIVE_REPORTING_LEAD_MS` is removed; `canStartLiveReporting()` now
offers the button for the whole life of an unplayed, unreported, non-cancelled match, with no
kickoff-proximity check at all. There is no safety reason to hide it earlier — a coach who opens
live reporting three days before kickoff by mistake can simply not use it; a coach who needs it
and can't find it is the actual failure mode this hardening programme exists to close. Today's
`live_report_available` work item (H4) is intentionally unaffected — it already has its own,
separate, narrower "a finalized squad exists" gate, and stays that way per the coach's own
request that only the Today surface should be gated, not the match view.
