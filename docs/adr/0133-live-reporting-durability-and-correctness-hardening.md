...
- **H6** is planned here and extends this ADR with a concrete design in its PR. It needs the
  Cloudflare Worker Observability logs for `noisy-snowflake-faf0` over the incident window
  (15:15–16:19 UTC 2026-09-09) to root-cause the 218-reconnect / ~53 s-median-connection churn —
  the API token still cannot read `workers/observability/telemetry`.
