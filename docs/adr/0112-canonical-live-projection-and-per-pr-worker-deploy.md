...
### 2026-09-03 (correction: the per-PR Worker deploy step described above was never actually added)

Found while investigating a "Deploy PR to Test slot fails every time now" report: `test-acceptance.yml` never actually gained a `wrangler deploy --env test` step — this ADR's Decision/History sections described intended work that did not ship in PR #401 despite the ADR being merged as "Accepted". Consequence: the `test`/`production` Cloudflare Workers were never redeployed after PR #401 changed `workers/live-match/src/match-session-object.ts`, because `deploy-live-match-worker.yml` only deploys after a CI-green push to `main`, and every push to `main` since PR #401 failed CI on the same `follow-live.spec.ts` E2E test — for an unrelated reason (see ARR-0039: a real `matchType`/`type` field-name bug in the Follow Live server page, independent of Worker staleness). The stale Worker was a real, separate, ~11-hour production issue (Follow Live's Worker-side behavior lagged `main` for both environments), fixed by manually triggering `deploy-live-match-worker.yml` via `workflow_dispatch`. The actually-missing step — "Deploy PR's Worker code to Test environment" (`npx wrangler deploy --env test`) plus a `LIVE_MATCH_INTERNAL_SECRET_TEST` sync, gated on `CLOUDFLARE_API_TOKEN`/`CLOUDFLARE_ACCOUNT_ID` being configured (skips gracefully, matching this ADR's own risk mitigation, if not) — was written and prepared as a follow-up change to `test-acceptance.yml` on 2026-09-03, but could not land in the same push because the coding agent's push credential lacked the `workflow` OAuth scope GitHub requires to push a `.github/workflows/*.yml` change directly. It stayed stashed, unapplied, until the fix below.

### 2026-09-03 (follow-up landed)

The per-PR Worker deploy step described above landed in `test-acceptance.yml`, applied from the stash prepared earlier the same day, unmodified, with a real `--check`/YAML-parse verification before commit. Raised proactively — no CI failure prompted this; the Evidence-Informed Match Planning programme's 8 bundles (PRs #402–#411) had just completed with no CI issues and none of them touched `workers/live-match/`, so the staleness risk this step closes was real but dormant. See this PR's own record for whether the push succeeded directly or required a maintainer's help, depending on the pushing credential's `workflow` OAuth scope at the time.