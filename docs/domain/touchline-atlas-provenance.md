...
## 39. Phase 8 — Brand asset convergence (2026-09-12)

`11_BRAND_ICON_AND_PWA_CONTRACT.md` in full: **§1 (no major rebrand)** — the mark's path geometry
in `public/brand/logo.svg` is byte-identical before and after this change (confirmed: only
`scripts/generate-pwa-icons.sh`'s two colour variables changed, never the SVG source or the
`convert`/`sed` commands' shape-affecting steps); no ball/crest/shield/pitch/mascot/text was
added; the product name is unchanged everywhere (`Matchboard`/`Matchboard Test`). **§2 (Touchline
treatment)** — "Preferred launcher: Touchline accent/lime background; existing mark in
near-black" is implemented exactly: background `--tl-accent` (`#C7F54A`, `touchline.css`'s dark
value — icons render this literal hex since the mark composition happens in a Node-independent
shell script with no CSS variable resolution, matching the existing pre-Touchline script's own
identical literal-hex approach); mark `--tl-accent-on-fill` dark value (`#101500`). This is one
color scheme for all icon assets, not a separate dark-UI-mark/light-UI-mark distinction — §2's
"Dark-theme UI mark"/"Light-theme UI mark" language describes the in-app `TouchlineMark`
component (which already inherits `currentColor` per-theme via CSS, unaffected by this change),
not the static app-icon files, which have exactly one appearance regardless of OS/browser theme
(standard PWA/favicon convention — manifests and `<link rel="icon">` do not carry per-theme icon
variants). **§3 (required assets)** — every named asset now uses this treatment:
`favicon.ico` (genuinely fixed, see below), `apple-icon.png` (180), `icon.png` (32),
`android-chrome-{192,512}.png` (`purpose: "any"`), `maskable-{192,512}.png`
(`purpose: "maskable"`). A scalable `icon.svg` was not added — confirmed the app architecture
does not use one (Next.js resolves `src/app/icon.png` via file convention; no route or metadata
config references an `icon.svg`), matching §3's own "if architecture uses it" qualifier. A
notification monochrome icon was not added — confirmed no push-notification implementation exists
anywhere in the codebase (Web Push is explicitly out of scope per AGENTS.md's PWA section,
"Custom Web Push (see also ADR-0086)"), matching §3's own "only if current push implementation
needs one" qualifier. **§4 (manifest/theme)** — `src/app/manifest.ts`'s `background_color`/
`theme_color` corrected from `#0a0d13` (a slightly-off pre-Touchline value) to `#090b0f`, the
exact `--tl-canvas` dark token `src/app/layout.tsx`'s own `viewport.themeColor` already used for
`prefers-color-scheme: dark` — the two had quietly drifted apart since ADR-0134/0135's earlier
Touchline passes touched `layout.tsx` but not `manifest.ts`. Icon `purpose` values (`any`/
`maskable`) were already correct and unchanged. No product rename — confirmed `name`/
`short_name` unchanged in both branches of `manifest.ts`'s hostname switch. **§5 (golden brand
board correction)** — `brand-asset-matrix.png` was inspected and its depicted mark confirmed to
differ from the repository's canonical `public/brand/logo.svg` geometry; per §5's own explicit
instruction, this was ignored and the repository's real mark was used, unmodified, throughout.

**A real, previously-unaddressed defect, of the same class ADR-0123 already fixed once**: while
auditing every icon asset for treatment consistency, `src/app/favicon.ico` was found to be a
transparent PNG-in-ICO containing only a plain black mark with no background fill — nearly
invisible against a dark browser tab bar, the exact defect ADR-0123 already diagnosed and fixed
for `apple-icon.png` ("iOS composites home-screen icons onto black, so a transparent icon renders
as black-on-black"). `generate-pwa-icons.sh` previously did not touch `favicon.ico` at all. Now
generated by the same script, same treatment, opaque, consistent with every other icon.

**Owner-approval-gate handling (`docs/product/brand-strategy.md`)**: gate 2 ("Final logo / app
icon... a coding agent must not conclude any of these autonomously") is a standing constraint for
*ordinary* feature/programme work. This change is explicitly authorized by a different, more
specific decision already on record: ADR-0136 is Accepted, and its own Phase 8 scope — "Create
minor icon variants from existing mark. Update PWA/favicon/theme integration" — together with
`11_BRAND_ICON_AND_PWA_CONTRACT.md`'s explicit Allowed/Not-allowed list (background/foreground
colour change is Allowed; redraw/reshape/new symbol is not) constitutes that gate's required
owner approval for this one, narrow, colour-only, mark-preserving change — not a fresh, unilateral
brand conclusion made in the course of ordinary work. A clarifying note was added directly to
`brand-strategy.md` itself (not just this record) so a future reader of that document sees the
same reasoning without needing to cross-reference this provenance doc. The gate remains open and
unaddressed for anything beyond this: a new mark, a colour outside the app's own already-approved
Touchline token set, or a product rename.

Verified: regenerated every asset via the updated `scripts/generate-pwa-icons.sh` (ImageMagick;
confirmed available and produces correct output in this environment), visually inspected the
composited output (lime background, near-black mark, correct relative mark-to-canvas scale
matching the pre-existing per-asset percentages), confirmed `android-chrome-*`/`maskable-*`/
`apple-icon.png`/`icon.png` are all now opaque with no alpha channel (PNG24) via `identify`, and
confirmed `favicon.ico`'s own alpha channel (the ICO container format always carries one, unlike
a flattened PNG24) is fully opaque throughout (`identify`'s per-pixel alpha min/max both report
full opacity) — no transparent pixel remains anywhere in the mark's former near-invisible state. A
new regression test (`src/app/__tests__/manifest.test.ts`) locks in the exact
