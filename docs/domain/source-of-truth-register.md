...
| `Selection.explanation` / `SelectionExplanation` table | Two storage locations for selection rationale | Round board, explanations | Generation engine | Content divergence | Yes (partial) | IMPROVE-0C: provenance columns added to Selection; Selection.explanation remains as display cache; SelectionExplanation is canonical for structured explanations |
