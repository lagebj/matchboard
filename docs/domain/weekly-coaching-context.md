...
|---|---|---|
| Available player without a planned league opportunity | `computeRoundPlanIntegrity()` (`src/lib/selection/compute-plan-integrity.ts`), rule `AVAILABLE_PLAYER_WITHOUT_PLANNED_OPPORTUNITY` | Calls it once for the round whose matches fall within this week's date range; extracts `playerId`s from that rule's signals, then resolves each to `{ displayName, href }` via a `db.player.findMany` (the signal itself carries only a `playerId`), keeping **only ids that resolve to a non-removed player** so the headline count always equals the rendered list. The rule only fires for a player whose core team has a non-cancelled match in that round — a player whose core team is not playing is an optional Round Board helper, never a carry-forward decision. For a week whose round is already FINALIZED, `computeRoundPlanIntegrity()` judges availability against the per-round snapshot frozen at planning-boundary close (ADR-0121), not a since-changed current value — so the carry-forward line reflects who was actually available that week. Never recomputes availability/eligibility itself. |
