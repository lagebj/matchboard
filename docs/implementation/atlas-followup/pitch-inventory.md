...
| Match detail → Tactics tab (`match-tactics-panel.tsx`, `MatchTacticsPanel`) | `TacticsBoard` mode `lineup-assignment`/`lineup-readonly` via `PitchLineupView` | same | **explicit `"horizontal"`** | flat (default) | click-to-assign via `PlayerPicker` dialog, positional-fit inspector | `TouchlinePlanningPitch` | Not started |
