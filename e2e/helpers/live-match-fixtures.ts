 ...
 * paradox odds climbed with every additional commit -- two separate CI runs on this PR hit
 * collisions (one "Finalised matches cannot be recalculated", one "date is outside the current
 * league season", and a third run repeated the first verbatim). The team already widened this
 * pool once before for the identical underlying mechanism (500 -> 5000 weeks); that margin was
 * sized for a typical PR's commit count, not an outlier this long. Fixed two ways instead of
 * widening the pool a third time: (1) `process.hrtime.bigint()` (nanosecond-resolution,
 * effectively never repeats within a process) replaces `Math.random()`, so two calls within the
 * same test run can no longer coincidentally draw the same date at all -- not just less likely
 * to, closing the exact retry-collision class documented above outright; (2) the pool itself is
 * widened another order of magnitude (5000 -> 50,000 weeks, ~960 years -- test dates only need
 * to be distinct, never plausible), cutting cross-run birthday-paradox odds by roughly 10x for
 * the same number of draws, so a long-lived PR needs roughly 10x more commits before the same
